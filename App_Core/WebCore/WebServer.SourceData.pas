      ...
      begin
        if (SL.Strings[0] = Trim(String(FTrans.Token))) then
          if (BC.Inquiries.TryGetAccountID(String(FTrans.Afrom)) = AccID) then
            Sent := Sent + FTrans.Sent
          else if (BC.Inquiries.TryGetAccountID(String(FTrans.Ato)) = AccID) then
            Received := Received + FTrans.Sent;
      end;

      Result.AddPair('symbol', Trim(SL.Strings[0]));
      Result.AddPair('received', FloatEToString(Received));
      Result.AddPair('sent', FloatEToString(Sent));
      Result.AddPair('balance', FloatEToString(BC.Inquiries.TryGetBalance(Address, SL.Strings[0])));
    end;
  finally
    SL.Clear;
    SL.Free;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        JSONNestedObject.AddPair('sent', FloatEToString(FPassedTransArray[i].Sent));
        JSONNestedObject.AddPair('received', FloatEToString(FPassedTransArray[i].Sent));
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          JSONNestedObject.AddPair('balance', FloatEToString(BC.Inquiries.TryGetBalance(Address, SL.Strings[i])));
...
try
  ...
  begin
    ...
    begin
      ...
      JSONNestedObject.AddPair('sent', FloatEToString(FAllTrans[i].Sent));
      JSONNestedObject.AddPair('received', FloatEToString(FAllTrans[i].Sent));
      JSONNestedObject.AddPair('fee', TJSONNumber.Create(0));
...
try
  ...
  begin
    ...
    begin
      ...
      JSONNestedObject.AddPair('sent', FloatEToString(TransArr[i].Sent));
      JSONNestedObject.AddPair('received', FloatEToString(TransArr[i].Sent));
      JSONNestedObject.AddPair('fee', TJSONNumber.Create(0));
  ...
  begin
    ...
    begin
      ...
      JSONNestedObject.AddPair('balance', FloatEToString(BC.Inquiries.TryGetBalance(OwnersArr[i].Address, Symbol)));
...
try
  ...
  Result.AddPair('sent', FloatEToString(THI.Sent));
  Result.AddPair('received', FloatEToString(THI.Sent));
...
try
  ...
  Balance := BC.Inquiries.TryGetBalance(Address, Token);
  if Balance = -1 then
    exit;

  Result.AddPair('wallet', Address);
  Result.AddPair('token', Token);
  Result.AddPair('balance', TJSONNumber.Create(Balance));
