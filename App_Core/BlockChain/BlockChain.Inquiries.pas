...
begin
  ...
  if Length(buf) = 0 then
    ...
    begin
      ...
      sd := GetVolumeFromAmount(item.Amount, TokensChain.GetTokenDecimals(item.TokenID));
      if item.Plus then
        Amount := FloatEToString(sd)
      else
        Amount := '-' + FloatEToString(sd);
            ...
            0:
              begin
                var
                  trx: TServiceV0;
                trx := ABytes;
                flag := trx.CheckTrx(TryGetPublicKey(trx.ServiceInfo.Owner));
                if not flag then
                begin
                  Result := False;
                  exit;
                ...
                BaseBlockBB := TServiceBlockV0.Create(trx, ServiceChain.GetLastBlockHash);
