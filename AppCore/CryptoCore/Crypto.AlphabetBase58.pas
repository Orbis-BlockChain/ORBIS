...
resourcestring
  ...
  SAlphabetParameterMismatch = 'Required Alphabet Length is "%d" but Provided Alphabet is "%d" Characters Long.';
  SInvalidCharacter = 'Alphabet Contains Character "%s (%d)" Above "%d"';
  SInvalidInput = 'Invalid character value in input "%s"';

type

  TCharArr = TArray<Char>;
  TByteArr = TBytes;
  ...
  TEncodingAlphabet = class abstract(TInterfacedObject)
  ...
  protected
    ...
    function GetReverseLookupTable: TByteArr; inline;

    property length: Int32 read GetLength;
    property value: String read GetValue;
    property ReverseLookupTable: TByteArr read GetReverseLookupTable;
...
begin
  ...
  begin
    raise EInvalidOperationSimpleBaseLibException.CreateResFmt(@SInvalidCharacter, [c, Ord(c), lookupLength]);
  end;
...
begin
  ...
  begin
    raise EArgumentNilSimpleBaseLibException.CreateResFmt(@SAlphabetNil, [alphabet]);
  end;

  if (System.length(alphabet) <> length) then
  begin
    raise EArgumentSimpleBaseLibException.CreateResFmt(@SAlphabetParameterMismatch, [length, System.length(alphabet)]);
  end;
...
function TEncodingAlphabet.GetReverseLookupTable: TByteArr;
begin
