...
begin
  case TOsversion.Platform of
    pfWindows:
      Result := TPath.Combine(GetCurrentDir, '.config');
    pfMacOS:
...
{$IFDEF MAINNET}
    Add('TypeNet', 'MAINNET').
    Add('staticIP','127.0.0.1').
    BeginArray('ConnectTo').
    Add(DefaultMainNetIPs[0]).
    EndArray.BeginArray('ApprovedConnections').
    EndArray.
    Add('ServerPort', DefaultMainNetPorts[0]).Add('ClientPort', DefaultMainNetPorts[1])
    .Add('WebServerPort', DefaultMainNetPorts[2]).
    Add('NodeState', 'FullNode').Add('Wallet', '').Add('Pass', '').
{$ENDIF}
{$IFDEF TESTNET}
    Add('TypeNet', 'TESTNET').Add('staticIP','127.0.0.1').BeginArray('ConnectTo').Add(DefaultTestNetIPs[0]).EndArray.BeginArray('ApprovedConnections')
    .EndArray.Add('ServerPort', DefaultTestNetPorts[0]).Add('ClientPort', DefaultTestNetPorts[1])
    .Add('WebServerPort', DefaultTestNetPorts[2]).Add('NodeState', 'FullNode').Add('Wallet', '').Add('Pass', '').
{$ENDIF}
{$IFDEF LABNET}
    Add('TypeNet', 'LABNET').Add('staticIP','127.0.0.1').BeginArray('ConnectTo').Add(DefaultLabNetIPs[0]).EndArray.BeginArray('ApprovedConnections')
    .EndArray.Add('ServerPort', DefaultLabNetPorts[0]).Add('ClientPort', DefaultLabNetPorts[1])
    .Add('WebServerPort', DefaultLabNetPorts[2]).Add('NodeState', 'FullNode').Add('Wallet', '').Add('Pass', '').
{$ENDIF}
    EndObject.BeginObject('APIConfig').Add('Server', false).Add('APIName', 'SomeName').Add('APIKey', 'SomeKey')
    .EndObject.EndObject;
...
begin
  ...
  try
    ...
    var NetState: string := JSON.AsString;
    ...
    begin
      JSON.Recurse;
      while JSON.Next do
        FConnectTo := FConnectTo + [JSON.AsString];
      JSON.Return;
    end;
