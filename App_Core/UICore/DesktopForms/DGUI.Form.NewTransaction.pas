...
type
  ...
  private
    ...
    procedure AddToken(const AName, AValueStr: String; ABalance: Extended);
...
procedure TNewTransactionForm.AddToken(const AName, AValueStr: String; ABalance: Extended);
var
  Item: TListBoxItem;
  TB: TTokenBalance;
  Txt: TText;
begin
  ...
  begin
    ...
    if (AName = 'OM') and (ABalance <> 0) then
      BoughtOM := True;
  end;
end;
...
begin
  if Pos('.', TokenEdit.Text) <> 0 then
    TokenEdit.MaxLength := 10
  else
    TokenEdit.MaxLength := 9;

  if TokenNameLabel.Text = 'OM' then
  begin
    if Tokens[Length(Tokens) - 1].Balance = 1 then
      TokenEdit.Text := '0'
    else
      TokenEdit.Text := '1';
  end
  ...
  begin
    ...
    if not TokenEdit.Text.IsEmpty then
      if StrToFloat(TokenEdit.Text.Replace(OldDecimalSeparator, DecimalSeparator)) > Tokens[TokensListBox.ListItems[TokensListBox.ItemIndex].Tag].Balance
      then
        TokenEdit.Text := Tokens[TokensListBox.ListItems[TokensListBox.ItemIndex].Tag].Balance.ToString.Replace(',', '.');
...
begin
  ...
  begin
    ...
    if (NoORBCLabel.Visible) and (Tokens[Length(Tokens) - 1].Balance = 1) then
      NoORBCLabel.Visible := False;
...
begin
  ...
  begin
    ...
    begin
      ...
      Inc(Counter);
      Value := StrToFloat(stringReplace(AArgs[Counter], OldDecimalSeparator, DecimalSeparator, [rfReplaceAll]));
      ValueStr := AArgs[Counter];
      Inc(Counter);
      AddToken(Symbol, ValueStr, Value);
    end;
  end;
  TokensListBox.EndUpdate;
